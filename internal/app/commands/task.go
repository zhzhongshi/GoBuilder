...
import (
	"bufio"
	"fmt"
	"github.com/KongchengPro/GoBuilder/internal/app"
...
func RunTask(projectPath, taskName string, taskArgs []string) (retErr error) {
	...
	log.Debug(tdk.MarshalArgs(&tdk.TaskCaller{ProjectPath: projectPath}))
	cmd := exec.Command(taskExecutableFilePath, tdk.MarshalArgs(&tdk.TaskCaller{ProjectPath: projectPath, Args: taskArgs}))
	log.WithField("cmd", cmd.Args).Debug()
